...
```{r atom 1 setup, include=FALSE}
atom1 <- get_tidy_audio_analysis("0NyBxQlgLlq6YgGEuDKekc")
```

```{r atom 1 tempogram, include=FALSE}
atom1_plot <- atom1 |>
...
```{r atom 2 setup, include=FALSE}
atom2 <- get_tidy_audio_analysis('47kqm43MxG45HEdGVudXxK')
```

```{r atom 2 tempogram, include=FALSE}
atom2_plot <- atom2 |>
  tempogram(window_size = 8, hop_size = 1, cyclic = FALSE) |>
  ggplot(aes(x = time, y = bpm, fill = power)) +
  geom_raster() +
...
```{r atom 3 setup, include=FALSE}
atom3 <- get_tidy_audio_analysis('5rlcAcJfUzssBjfZryvmRS')
```

```{r atom 3 tempogram, include=FALSE}
atom3_plot <- atom3 |>
  tempogram(window_size = 8, hop_size = 1, cyclic = FALSE) |>
  ggplot(aes(x = time, y = bpm, fill = power)) +
  geom_raster() +
...
```{r atom 4 setup, include=FALSE}
atom4 <- get_tidy_audio_analysis('3OQumqHQWrrRmCMlONe51e')
```

```{r atom 4 tempogram, include=FALSE}
atom4_plot <- atom4 |>
  tempogram(window_size = 8, hop_size = 1, cyclic = FALSE) |>
  ggplot(aes(x = time, y = bpm, fill = power)) +
  geom_raster() +
...
```{r atom 5 setup, include=FALSE}
atom5 <- get_tidy_audio_analysis('3zjpAA7IC5Hfm9hvBad86i')
```

```{r atom 5 tempogram, include=FALSE}
atom5_plot <- atom5 |>
  tempogram(window_size = 8, hop_size = 1, cyclic = FALSE) |>
  ggplot(aes(x = time, y = bpm, fill = power)) +
  geom_raster() +
...
```{r atom 6 setup, include=FALSE}
atom6 <- get_tidy_audio_analysis('4Hctp8jfbMwLTchI6o3vDc')
```

```{r atom 6 tempogram, include=FALSE}
atom6_plot <- atom6 |>
  tempogram(window_size = 8, hop_size = 1, cyclic = FALSE) |>
  ggplot(aes(x = time, y = bpm, fill = power)) +
  geom_raster() +
...
```{r atom 7 setup, include=FALSE}
atom7 <- get_tidy_audio_analysis('3KD3qVfZd6C7ERAaCYkhTn')
```

```{r atom 7 tempogram, include=FALSE}
atom7_plot <- atom7 |>
  tempogram(window_size = 8, hop_size = 1, cyclic = FALSE) |>
  ggplot(aes(x = time, y = bpm, fill = power)) +
  geom_raster() +
...
```{r atom 8 setup, include=FALSE}
atom8 <- get_tidy_audio_analysis('3HOe1ZIcGx29Q8SGaYmDH2')
```

```{r atom 8 tempogram, include=FALSE}
atom8_plot <- atom8 |>
  tempogram(window_size = 8, hop_size = 1, cyclic = FALSE) |>
  ggplot(aes(x = time, y = bpm, fill = power)) +
  geom_raster() +
...
```{r atom 9 setup, include=FALSE}
atom9 <- get_tidy_audio_analysis('2flsa9qZKC4j6oAKwoBZc0')
```

```{r atom 9 tempogram, include=FALSE}
atom9_plot <- atom9 |>
  tempogram(window_size = 8, hop_size = 1, cyclic = FALSE) |>
  ggplot(aes(x = time, y = bpm, fill = power)) +
  geom_raster() +
...
grid.arrange(atom1_plot, atom2_plot, atom3_plot, atom4_plot, atom5_plot, atom6_plot, atom7_plot, atom8_plot, atom9_plot, nrow=3)
```
...
In _Atom 1_, we hear a lof of different sounds coming together. It can be heard that the tempo is certianly not constant. This can also be seen in the tempogram.
...
### Tempogram _Everything she does is magic_ by _Sleeping At Last
```{r magic setup, echo=FALSE}
...
tempo + theme + theme(panel.grid = element_blank(), legend.position=c(0.8,0.9))
```
